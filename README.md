...
Filesystems embedded in the A4091 ROM are loaded **on demand**: when the mounter finds a device or partition whose DosType has no matching entry in `FileSystem.resource`, it asks the driver to load a matching filesystem from the ROM (or to initialize a Kickstart-resident CDFS). Media and partitions that don't need a ROM filesystem never cause one to be loaded - so with "CDROM Boot" disabled in the boot menu, no CD filesystem is registered at all, and a disk-based handler named in a mountlist (`FileSystem=` line) is genuinely used instead of being shadowed by the ROM. This is what allows for seamless booting from a CD or automatic access to FAT-formatted drives without polluting `FileSystem.resource` on systems that don't use them.

Filesystems are matched by the DosType recorded in the ROM slot (`romtool -T`), which must match the DosType the filesystem actually serves. A filesystem is loaded when matching *media* is detected at boot: a data CD in the drive, or a partition of that DosType. If the CD drive was empty at boot, no CD device node is created and the ROM filesystem stays unloaded; mounting a CD inserted later then requires a mountlist with a `FileSystem=` line pointing to a disk-based handler (the DOSDriver shipped with ODFileSystem does exactly this).
