...
Filesystems are matched by the DosType recorded in the ROM slot (`romtool -T`), which must match the DosType the filesystem actually serves. After the ROM entry is removed, mounting later media requires a mountlist with a `FileSystem=` line pointing to a disk-based handler; the DOSDriver shipped with ODFileSystem does exactly this. Older Kickstarts retain the ROM filesystem for the rest of the boot.
